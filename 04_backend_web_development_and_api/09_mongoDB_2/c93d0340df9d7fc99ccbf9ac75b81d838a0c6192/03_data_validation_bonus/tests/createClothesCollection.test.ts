import { clothesValidator, sweaterProperties, shoesProperties, pantsProperties } from "../src/createClothesCollection";
...
describe("clothesValidator", () => {
  ...
  it("Should have a property oneOf with an array containing the 3 clothes properties", () => {
    expect(Array.isArray(clothesValidator.validator.$jsonSchema.oneOf)).toBe(true);
  ...
  describe("sweaterProperties", () => {
    ...
    describe("size", () => {
      ...
      it(`Should be only a string in ["XXS", "XS", "S", "M", "L", "XL", "XXL", "XXXL"]`, () => {
        ...
        const acceptedValues = ["XXS", "XS", "S", "M", "L", "XL", "XXL", "XXXL"];
  ...
  describe("pantsProperties", () => {
    ...
    describe("size", () => {
      ...
      it("Should not accept additional properties", () => {
        expect(pantsProperties.properties.size.additionalProperties).toBe(false);
      });
      ...
      describe("width", () => {
        ...
        it("Should be only an 'integer' between 32 and 46", () => {
          expect(pantsProperties.properties.size.properties.width.bsonType).toBe("int");
          expect(pantsProperties.properties.size.properties.width.minimum).toBe(32);
          expect(pantsProperties.properties.size.properties.width.maximum).toBe(46);
        });
      });
      ...
      describe("cut", () => {
        ...
        it(`Should be only a 'string' in ["slim", "skinny", "regular", "straight"]`, () => {
          const validatorEnum = pantsProperties.properties.size.properties.cut.enum;

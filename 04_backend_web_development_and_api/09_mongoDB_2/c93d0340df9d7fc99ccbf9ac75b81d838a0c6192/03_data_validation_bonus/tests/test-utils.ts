...
export const randomInt = (max: number): number => Math.floor(Math.random() * Math.floor(max));
...
export async function dropCollection(db: Db, collectionName: string): Promise<boolean> {
  return new Promise((resolve, reject) => {
...
export async function dropCollections(db: Db, collections: string[]): Promise<boolean[]> {
